function [errors MSE IterTimes]=MIMO_RecvComSupt(SendCell, RecvCell, ChnlCell, DecodeStruct, SysSturct, init, N_it, MeasureMSE,EstNoisePrec)
% Author: YUAN
%function:MIMO-OFDM receiver, common support
%author: yuan
%time: 2016.5.6PM
% 注释：导频信道估计采用串行schedule，全局信道估计采用并行schedule。
%
...
%% %%%%%%%%%%%%%%%%%%%%%%Pilot Iterations%%%%%%%%%%%%%%%%%%%%%
%单纯导频信道估计
%[ChnlEstCell LmdEst]=SparseChnlEstPilot_Par(SysSturct,SendCell,RecvCell,ChnlCell,DecodeStruct,Lmd,EstNoisePrec,Niter);
[ChnlEstCell LmdEst IterTimes]=SparseChnlEstPilot_Ser(SysSturct,SendCell,RecvCell,ChnlCell,DecodeStruct,Lmd,EstNoisePrec,Niter);
if EstNoisePrec
    Lmd=LmdEst;
end
for m=1:M
...
while it <= N_it && ( errors(it-1) || MeasureMSE )
    ...
    %% %%%%%%%%%%%%%%%%%%联合信道估计%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [ChnlEstCell]=SparseChnlEstGlobal_Par(SysSturct,ChnlEstCell,ChnlCell,DecodeStruct,1); %并行schedule
    H=ChnlCell{1,1}.H;
    MSE(1,it) = mean(abs(ChnlEstCell{1,1}.blf_h_m(dataSubcIndx)-H(dataSubcIndx)).^2); %观察信道估计MSE
    %     %联合信道估计时候采用串行schedule
    %     [ChnlEstCell_Se]=SparseChnlEstGlobal_Ser(SysSturct,ChnlEstCell,ChnlCell,DecodeStruct,1);
    %     H=ChnlCell{1,1}.H;
    %     MSE_Se(1,it) = mean(abs(ChnlEstCell_Se{1,1}.blf_h_m(dataSubcIndx)-H(dataSubcIndx)).^2); %观察信道估计MSE
    %% %%%第二次，估计噪声方差，得到b（h）以后%%%%
    if EstNoisePrec
        Lmd=LmdEstimation(SysSturct,DecodeStruct,RecvCell,SendCell,ChnlEstCell,XnCell); %更新lembda,尚未完成！
    end
